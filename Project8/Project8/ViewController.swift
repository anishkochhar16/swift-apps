...
//        buttonsView.layer.borderColor = UIColor.lightGray.cgColor
    ...
    @objc func submitTapped(_ sender: UIButton) {
        ...
        if let solutionsPosition = solutions.firstIndex(of: answerText) {
            ...
            score += 1
            questionsRight += 1
            if questionsRight % 7 == 0 {
                let ac = UIAlertController(title: "Well done!", message: "Are you ready for the next level?", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Let's go!", style: .default, handler: levelUp))
                present(ac, animated: true)
            }
        } else {
            let ac = UIAlertController(title: "\(answerText) is not a correct answer", message: "You have been deducted 1 point", preferredStyle: .alert)
    ...
    @objc func loadLevel() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if self?.letterButtons.count == letterBits.count {
                let letterCount = self?.letterButtons.count
